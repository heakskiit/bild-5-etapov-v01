...
import { getProductsByGame } from '@/lib/catalog';
import { getTranslations } from '@/lib/i18n/getTranslations';

/** GTA V — the three real subsections: Level Boost / Money / Shark Cards. */
export default async function Gta5Page({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations();
  const products = getProductsByGame('gta5');

  return (
    <div>
      <h1 className="font-display text-3xl text-neon-pink">{t('nav.gta5')}</h1>
